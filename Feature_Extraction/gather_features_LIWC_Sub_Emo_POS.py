...
data_dir = "C:\\Users\\James\\Google Drive\\Grad Research\\AffectCon Shared Task\\claff-happydb-master\\data\\TRAIN"
data_f = "labeled_10k.csv"

df = pd.read_csv(os.path.join(data_dir,data_f))

all_data = list(df["moment"])

#Need to normalize, then predict, then find text sim
...
for text in all_data:    
    feats = get_feats(text)
    feat_vals = []
    feats["Strong-subjective"] = feats["strong-negative"] + feats["strong-positive"]
    feats["Weak-subjective"] = feats["weak-negative"] + feats["weak-positive"]

    for feat in feat_list_model_order:
        feat_vals.append(feats[feat])
    all_data_feats.append(feat_vals)
    progress += 1
    temp_p = int((progress/total)*100)
    if temp_p != old:
        old = temp_p
        print("%s%%" % old)

#Convert the input data to a numpy array
#all_data_feats = np.array(all_data_feats)
data_feats_df = pd.DataFrame(all_data_feats,columns=feat_list_model_order)

#face_data = normalize(np.array(face_data), norm='l2')

#Apply a L2 normalization to the input data
data_feats_normed = normalize(np.array(all_data_feats), norm='l2')

#Run the model on the input data
r_results = r_regr.predict(data_feats_normed)
data_feats_df["Fact-Feel"] = pd.Series(r_results)
data_feats_df["moment"] = df["moment"]

data_feats_df.to_csv("labeled_10k_features.csv")
#r_regr_results = {}
#for idx,text in enumerate(all_data):
#    r_regr_results[text] = r_results[idx]
